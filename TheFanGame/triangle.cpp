...
triangle::triangle(const sf::Vector2f& size, const sf::Vector2f& pos, const sf::Color& color) noexcept
{
	this->m_sprites.setPrimitiveType(sf::PrimitiveType::Triangles);
	this->m_sprites.resize(3);

	this->m_vertices.setPrimitiveType(sf::PrimitiveType::Triangles);
	this->m_vertices.resize(3);

	this->m_vertices[0].position = sf::Vector2f(size.x - 3.f, size.y);
	this->m_vertices[1].position = sf::Vector2f(size.x + 3.f, size.y);
	this->m_vertices[2].position = sf::Vector2f(size.x, size.y + 5.f);
...
const void triangle::update(entity& player, const sf::Vector2u& windowSize, const std::vector<float>& zBuffer) noexcept
{
    ...
    {
        this->m_sprites[0].position = sf::Vector2f((float)lastX, (float)drawStartY);
        this->m_sprites[1].position = sf::Vector2f((float)firstX, (float)drawStartY);
        this->m_sprites[2].position = sf::Vector2f((float)(lastX + firstX) / 2.f, (float)drawEndY);
...
const void triangle::setSize(const sf::IntRect& size) noexcept
{
}
