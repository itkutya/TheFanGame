...
{
	this->m_frontImage.setTexture(resourceSystem::c_get<sf::Texture>("FrontImage"));
	this->m_frontImage.setTextureRect(sf::IntRect(sf::Vector2i(600 * settings::m_currFrontPicture, 0), sf::Vector2i(600, 600)));

	this->m_icon.setTexture(resourceSystem::c_get<sf::Texture>("Icon"));
	this->m_icon.setTextureRect(sf::IntRect(sf::Vector2i(100 * settings::m_currProfilePicture, 0), sf::Vector2i(100, 100)));

	this->m_MainMusic = resourceSystem::get<std::unique_ptr<sf::Music>>(settings::m_currMusic).get();
}

const void settingsPanel::init(window& window) noexcept
{
}

const void settingsPanel::processEvent(const sf::Event& event) noexcept
{
}

const void settingsPanel::update(window& window, const sf::Time& dt) noexcept
{
	popup settingsPopUp("Settings", window.getWindow(), ImGuiWindowFlags_MenuBar);
...
const void settingsPanel::draw(window& window) noexcept
{
}
